...
![render-simple-map](quick-started/render-simple-map.png)
...
![render-colorful-map](quick-started/render-colorful-map.png)
...
[create-geoms.js](geometries/create-geom.js) represents how to create point, polygon and line geometries; then set a specific style base on various geometry type and render them into an image.

![create-geoms](geometries/create-geoms.png)

#### Buffer geometry by distance

[spatial-buffer.js](geometries/spatial-buffer.js) represents how to buffer a geometry with a specific distance. See the following images for difference.

* Buffers with `4` degrees
    ![spatial-buffer](geometries/spatial-buffer.png)
* Buffers with `20` degrees
    ![spatial-buffer](geometries/spatial-buffer-1.png)
...
* [create-geoms.js](geometries/create-geom.js)
...
[use-value-style.js](styles/use-value-style.js) represents how to set an exclusive fill colors based on field `CODE`.

![use-value-style](styles/use-value-style.png)

##### Filter and render areas based on some specific values

[render-filter-values.js](styles/render-filter-values.js) represents how to filter and render areas with field values `MAL`,  `ZAI` and `SUD` only.

![render-filter-values](styles/render-filter-values.png)
...
![use-text-style](styles/use-text-style.png)

### Layers and Sources

As the samples represented previously, in the mean time, the `FeatureSource` and `FeatureLayer` are also used in the previous demos (e.g. [render-colorful-map.js](quick-started/render-colorful-map.js)). Here I just emphasize there difference. 
